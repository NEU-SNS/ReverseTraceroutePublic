...
// func (s source) Get(serviceID router.service) (router.ServiceDef, error) {
// 	return s.SD, nil
// }
...
func TestGetMT(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// r.SetSource(test.s)
		sd, err := r.GetService(router.PlanetLab)
		if err != test.errs {
			t.Fatalf("r.GetService(%s), Expected[%v], Got[%v]", test.addr, test.errs, err)
		}
		mt, err := r.GetMT(sd)
		if err != test.errmt {
			t.Fatalf("r.GetMT(%s), Expected[%v], Got[%v]", sd.Addr, test.errmt, err)
		}
		if test.close {
			mt.Close()
		}
